...
import com.easy.common.commonutils.DisplayUtil;
...
class PowerfulController {
    ...
    public PowerfulController(CustomDialog CustomDialog, Window window) {
    ...
    public static class AlertParams{
        ...
        public SparseArray<String> mStringImgArray=new SparseArray<>();//存放图片string
...
//        public SparseArray<WeakReference<View.OnClickListener>> mClickArray=new SparseArray<>();//存放点击事件
        ...
        public void apply(PowerfulController mAlert) {
            ...
            //设置图片（针对于ImageView）
            int stringImgarrySize=mStringImgArray.size();
            for (int i=0;i<stringImgarrySize;i++){

                mAlert.setImage(mStringImgArray.keyAt(i),mStringImgArray.valueAt(i));
            }
            //设置点击
            ...
            if(pHeight>0){
                params.height= (int) (DisplayUtil.getScreenHeight(mContext)*pHeight);
            }else{
                params.width=mWidth;
            }
            if(pWidth>0){
                params.width= (int) (DisplayUtil.getScreenWidth(mContext)*pWidth);
            }else{
